...
public:
	...
	virtual void OnAttach() override
	{
		...
		m_MeshMaterial.reset(new susumu::MaterialInstance(m_Mesh->GetMaterial()));

		m_QuadShader = susumu::Shader::Create("assets/shaders/quad.glsl");
		m_HDRShader = susumu::Shader::Create("assets/shaders/hdr.glsl");

		m_SphereMesh.reset(new susumu::Mesh("assets/models/Sphere1m.fbx"));
		m_PlaneMesh.reset(new susumu::Mesh("assets/models/Plane1m.obj"));

		m_GridShader = susumu::Shader::Create("assets/shaders/Grid.glsl");
		m_GridMaterial = susumu::MaterialInstance::Create(susumu::Material::Create(m_GridShader));
		...
		for (int i = 0; i < 8; i++)
		{
			susumu::Ref<susumu::MaterialInstance> mi(new susumu::MaterialInstance(m_SphereMesh->GetMaterial()));
	...
	virtual void OnUpdate(susumu::Timestep ts) override
	{
		// THINGS TO LOOK AT:
		// - BRDF LUT
	...
	virtual void OnImGuiRender() override
	{
		...
		auto [wx, wy] = susumu::App::Get().GetWindow().GetWindowPos();
		posX -= wx;
		posY -= wy;
		SU_INFO("{0}, {1}", posX, posY);*/
...
private:
	susumu::Ref<susumu::Shader> m_QuadShader;
	susumu::Ref<susumu::Shader> m_HDRShader;
	susumu::Ref<susumu::Shader> m_GridShader;
	susumu::Ref<susumu::Mesh> m_Mesh;
	susumu::Ref<susumu::Mesh> m_SphereMesh, m_PlaneMesh;
	susumu::Ref<susumu::Texture2D> m_BRDFLUT;

	susumu::Ref<susumu::MaterialInstance> m_MeshMaterial;
	susumu::Ref<susumu::MaterialInstance> m_GridMaterial;
