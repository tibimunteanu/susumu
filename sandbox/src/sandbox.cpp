...
class EditorLayer : public susumu::Layer
{
public:
    EditorLayer()
    ...
    virtual void OnAttach() override
    {
        ...
        m_SimplePBRShader.reset(susumu::Shader::Create("assets/shaders/simplepbr.glsl"));
        m_QuadShader.reset(susumu::Shader::Create("assets/shaders/quad.glsl"));
        m_HDRShader.reset(susumu::Shader::Create("assets/shaders/hdr.glsl"));
        ...
        // Editor
        m_CheckerboardTex.reset(susumu::Texture2D::Create("assets/editor/Checkerboard.tga"));

        // Environment
        m_EnvironmentCubeMap.reset(susumu::TextureCube::Create("assets/textures/environments/Arches_E_PineTree_Radiance.tga"));
        //m_EnvironmentCubeMap.reset(susumu::TextureCube::Create("assets/textures/environments/DebugCubeMap.tga"));
        m_EnvironmentIrradiance.reset(susumu::TextureCube::Create("assets/textures/environments/Arches_E_PineTree_Irradiance.tga"));
        m_BRDFLUT.reset(susumu::Texture2D::Create("assets/textures/BRDF_LUT.tga"));

        m_Framebuffer.reset(susumu::Framebuffer::Create(1280, 720, susumu::FramebufferFormat::RGBA16F));
        m_FinalPresentBuffer.reset(susumu::Framebuffer::Create(1280, 720, susumu::FramebufferFormat::RGBA8));

        m_PBRMaterial.reset(new susumu::Material(m_SimplePBRShader));

        float x = -4.0f;
        float roughness = 0.0f;
        for (int i = 0; i < 8; i++)
        {
            susumu::Ref<susumu::MaterialInstance> mi(new susumu::MaterialInstance(m_PBRMaterial));
        ...
        m_VertexBuffer.reset(susumu::VertexBuffer::Create());
        m_VertexBuffer->SetData(data, 4 * sizeof(QuadVertex));

        uint32_t* indices = new uint32_t[6]{ 0, 1, 2, 2, 3, 0, };
        m_IndexBuffer.reset(susumu::IndexBuffer::Create());
    ...
    virtual void OnUpdate(susumu::Timestep dt) override
    {
        ...
        if (m_AlbedoInput.TextureMap) m_PBRMaterial->Set("u_AlbedoTexture", m_AlbedoInput.TextureMap);
        if (m_NormalInput.TextureMap) m_PBRMaterial->Set("u_NormalTexture", m_NormalInput.TextureMap);
        if (m_MetalnessInput.TextureMap) m_PBRMaterial->Set("u_MetalnessTexture", m_MetalnessInput.TextureMap);
        if (m_RoughnessInput.TextureMap) m_PBRMaterial->Set("u_RoughnessTexture", m_RoughnessInput.TextureMap);
        ...
        if (m_Scene == Scene::Spheres)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                m_SphereMesh->Render(dt, m_SimplePBRShader.get());
            }
        ...
        else if (m_Scene == Scene::Model)
        {
            ...
            {
                m_PBRMaterial->Bind();
                m_Mesh->Render(dt, m_SimplePBRShader.get());
            }
        }
        ...
        m_PlaneMesh->Render(dt, m_GridShader.get());
    ...
    virtual void OnImGuiRender() override
    {
        ...
        ImGui::Separator();
        {
            ...
            {
                std::string filename = susumu::App::Get().OpenFile("");
                if (filename != "")
                    m_Mesh.reset(new susumu::Mesh(filename));
            }
        }
        ImGui::Separator();
            ...
            if (ImGui::CollapsingHeader("Albedo", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
            {
                ...
                if (ImGui::IsItemHovered())
                {
                    ...
                    if (ImGui::IsItemClicked())
                    {
                        std::string filename = susumu::App::Get().OpenFile("");
                        if (filename != "")
                            m_AlbedoInput.TextureMap.reset(susumu::Texture2D::Create(filename, m_AlbedoInput.SRGB));
                    }
                }
                ImGui::SameLine();
                ImGui::BeginGroup();
                ...
                {
                    if (m_AlbedoInput.TextureMap)
                        m_AlbedoInput.TextureMap.reset(susumu::Texture2D::Create(m_AlbedoInput.TextureMap->GetPath(), m_AlbedoInput.SRGB));
                }
                ImGui::EndGroup();
                ImGui::SameLine();
            ...
            if (ImGui::CollapsingHeader("Normals", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
            {
                ...
                if (ImGui::IsItemHovered())
                {
                    ...
                    if (ImGui::IsItemClicked())
                    {
                        std::string filename = susumu::App::Get().OpenFile("");
                        if (filename != "")
                            m_NormalInput.TextureMap.reset(susumu::Texture2D::Create(filename));
                    }
                }
                ImGui::SameLine();
            ...
            if (ImGui::CollapsingHeader("Metalness", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
            {
                ...
                if (ImGui::IsItemHovered())
                {
                    ...
                    if (ImGui::IsItemClicked())
                    {
                        std::string filename = susumu::App::Get().OpenFile("");
                        if (filename != "")
                            m_MetalnessInput.TextureMap.reset(susumu::Texture2D::Create(filename));
                    }
                }
                ImGui::SameLine();
            ...
            if (ImGui::CollapsingHeader("Roughness", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
            {
                ...
                if (ImGui::IsItemHovered())
                {
                    ...
                    if (ImGui::IsItemClicked())
                    {
                        std::string filename = susumu::App::Get().OpenFile("");
                        if (filename != "")
                            m_RoughnessInput.TextureMap.reset(susumu::Texture2D::Create(filename));
                    }
                }
                ImGui::SameLine();
        ...
        {
            auto& shaders = susumu::Shader::s_AllShaders;
    ...
    virtual void OnEvent(susumu::Event& event) override
    {
    }
private:
    susumu::Ref<susumu::Shader> m_SimplePBRShader;
    susumu::Scope<susumu::Shader> m_QuadShader;
    susumu::Scope<susumu::Shader> m_HDRShader;
    susumu::Scope<susumu::Shader> m_GridShader;
    susumu::Scope<susumu::Mesh> m_Mesh;
    susumu::Scope<susumu::Mesh> m_SphereMesh, m_PlaneMesh;
    susumu::Ref<susumu::Texture2D> m_BRDFLUT;

    susumu::Ref<susumu::Material> m_PBRMaterial;
    std::vector<susumu::Ref<susumu::MaterialInstance>> m_MetalSphereMaterialInstances;
    std::vector<susumu::Ref<susumu::MaterialInstance>> m_DielectricSphereMaterialInstances;
    ...
    struct AlbedoInput
    {
        ...
        susumu::Ref<susumu::Texture2D> TextureMap;
        bool SRGB = true;
    ...
    std::unique_ptr<susumu::Framebuffer> m_Framebuffer, m_FinalPresentBuffer;

    susumu::Ref<susumu::VertexBuffer> m_VertexBuffer;
    susumu::Ref<susumu::IndexBuffer> m_IndexBuffer;
    susumu::Ref<susumu::TextureCube> m_EnvironmentCubeMap, m_EnvironmentIrradiance;

    susumu::Camera m_Camera;
    ...
    // Editor resources
    susumu::Ref<susumu::Texture2D> m_CheckerboardTex;
};
