...
namespace susumu
{
        ...
        : m_FilePath(path)
    {
        ...
        SU_CORE_INFO("Loading texture {0}, srgb={1}", path, srgb);
        ...
        SU_RENDER_S1(srgb, {
            ...
            if (srgb)
            {
                ...
                int32_t levels = CalculateMipMapCount(self->m_Width, self->m_Height);
                SU_CORE_INFO("Creating srgb texture width {0} mips", levels);
            ...
            else
            {
                ...
                glTexImage2D(GL_TEXTURE_2D, 0, ToOpenGLTextureFormat(self->m_Format), self->m_Width, self->m_Height, 0, srgb ? GL_SRGB8 : ToOpenGLTextureFormat(self->m_Format), GL_UNSIGNED_BYTE, self->m_ImageData.Data);
                glGenerateMipmap(GL_TEXTURE_2D);
    ...
    void OpenGLTexture2D::Unlock()
    {
        m_Locked = false;
        SU_RENDER_S({
            glTextureSubImage2D(self->m_RendererID, 0, 0, 0, self->m_Width, self->m_Height, ToOpenGLTextureFormat(self->m_Format), GL_UNSIGNED_BYTE, self->m_ImageData.Data);
            });
    }

    void OpenGLTexture2D::Resize(uint32_t width, uint32_t height)
    {
        SU_CORE_ASSERT(m_Locked, "Texture must be locked!");
        ...
        : m_FilePath(path)
    {
        ...
        SU_CORE_ASSERT(faceWidth == faceHeight, "Non-square faces!");
        ...
        SU_RENDER_S3(faces, faceWidth, faceHeight, {
            glGenTextures(1, &self->m_RendererID);
