...
namespace susumu {
	...
	struct LogStream : public Assimp::LogStream
	{
		...
		{
			SU_CORE_ERROR("Assimp error: {0}", message);
		}
	};
		...
		: m_FilePath(filename)
	{
		...
		SU_CORE_INFO("Loading mesh: {0}", filename.c_str());
		
		m_Importer = std::make_unique<Assimp::Importer>();

		const aiScene* scene = m_Importer->ReadFile(filename, s_MeshImportFlags);
		if (!scene || !scene->HasMeshes())
			SU_CORE_ERROR("Failed to load mesh file: {0}", filename);

		m_IsAnimated = scene->mAnimations != nullptr;
		m_MeshShader = m_IsAnimated ? Renderer::GetShaderLibrary()->Get("susumuPBR_Anim") : Renderer::GetShaderLibrary()->Get("susumuPBR_Static");
		m_Material.reset(new susumu::Material(m_MeshShader));
		...
		for (size_t m = 0; m < scene->mNumMeshes; m++)
		{
			...
			SU_CORE_ASSERT(mesh->HasPositions(), "Meshes require positions.");
			SU_CORE_ASSERT(mesh->HasNormals(), "Meshes require normals.");
			...
			{
				SU_CORE_ASSERT(mesh->mFaces[i].mNumIndices == 3, "Must have 3 indices.");
		...
		if (m_IsAnimated)
		{
			for (size_t m = 0; m < scene->mNumMeshes; m++)
			{
				...
				for (size_t i = 0; i < mesh->mNumBones; i++)
				{
					...
					{
						SU_CORE_TRACE("Found existing bone in map");
	...
	void Mesh::TraverseNodes(aiNode* node, int level)
	{
		...
		for (int i = 0; i < level; i++)
			levelText += "-";
		SU_CORE_TRACE("{0}Node name: {1}", levelText, std::string(node->mName.data));
	...
	{
		SU_CORE_ASSERT(pNodeAnim->mNumRotationKeys > 0);
	...
	{
		SU_CORE_ASSERT(pNodeAnim->mNumScalingKeys > 0);
	...
	glm::vec3 Mesh::InterpolateTranslation(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		SU_CORE_ASSERT(NextPositionIndex < nodeAnim->mNumPositionKeys);
		float DeltaTime = (float)(nodeAnim->mPositionKeys[NextPositionIndex].mTime - nodeAnim->mPositionKeys[PositionIndex].mTime);
		float Factor = (animationTime - (float)nodeAnim->mPositionKeys[PositionIndex].mTime) / DeltaTime;
		if (Factor < 0.0f)
			Factor = 0.0f;
		SU_CORE_ASSERT(Factor <= 1.0f, "Factor must be below 1.0f");
	...
	glm::quat Mesh::InterpolateRotation(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		SU_CORE_ASSERT(NextRotationIndex < nodeAnim->mNumRotationKeys);
	...
	glm::vec3 Mesh::InterpolateScale(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		SU_CORE_ASSERT(nextIndex < nodeAnim->mNumScalingKeys);
	...
	void Mesh::Render(Timestep ts, const glm::mat4& transform, Ref<MaterialInstance> materialInstance)
	{
		...
		SU_RENDER_S2(transform, materialOverride, {
	...
	void Mesh::DumpVertexBuffer()
	{
		...
		SU_CORE_TRACE("------------------------------------------------------");
		SU_CORE_TRACE("Vertex Buffer Dump");
		SU_CORE_TRACE("Mesh: {0}", m_FilePath);
		if (m_IsAnimated)
		{
			for (size_t i = 0; i < m_AnimatedVertices.size(); i++)
			{
				auto& vertex = m_AnimatedVertices[i];
				SU_CORE_TRACE("Vertex: {0}", i);
				SU_CORE_TRACE("Position: {0}, {1}, {2}", vertex.Position.x, vertex.Position.y, vertex.Position.z);
				SU_CORE_TRACE("Normal: {0}, {1}, {2}", vertex.Normal.x, vertex.Normal.y, vertex.Normal.z);
				SU_CORE_TRACE("Binormal: {0}, {1}, {2}", vertex.Binormal.x, vertex.Binormal.y, vertex.Binormal.z);
				SU_CORE_TRACE("Tangent: {0}, {1}, {2}", vertex.Tangent.x, vertex.Tangent.y, vertex.Tangent.z);
				SU_CORE_TRACE("TexCoord: {0}, {1}", vertex.Texcoord.x, vertex.Texcoord.y);
				SU_CORE_TRACE("--");
			}
		}
		else
